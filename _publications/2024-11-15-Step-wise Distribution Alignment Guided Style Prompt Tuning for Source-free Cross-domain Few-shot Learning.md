---
title: "Step-wise Distribution Alignment Guided Style Prompt Tuning for Source-free Cross-domain Few-shot Learning"
collection: Preprint
category: manuscripts
excerpt: 'This paper proposes a novel approach named Step-wise Distribution Alignment Guided Style Prompt Tuning (StepSPT) to implicitly narrow the domain gaps from the perspective of prediction distribution optimization.'
date: 2024-11-15
venue: 'arXiv'
slidesurl: 'https://github.com/xuhuali-mxj/StepSPT'
paperurl: 'https://arxiv.org/pdf/2411.10070'
citation: 'Xu, Huali, et al. "Step-wise Distribution Alignment Guided Style Prompt Tuning for Source-free Cross-domain Few-shot Learning." arXiv preprint arXiv:2411.10070 (2024).'
---

This paper proposes a novel approach named Step-wise Distribution Alignment Guided Style Prompt Tuning (StepSPT) to implicitly narrow the domain gaps from the perspective of prediction distribution optimization.
